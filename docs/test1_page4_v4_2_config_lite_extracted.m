classdef test1_page4_v4_2 < matlab.apps.AppBase
    ...
    properties (Access = private)
        ...
        PlotterRefreshPeriod double = 0.05   % 20 Hz
        MinMonitorRequestInterval double = 0.1
        AutoFollowPlots logical = true
        PlotShiftStep double = 10
        VisiblePlotPoints double = 500
        ...
        % Lightweight JSON configuration layer
        ConfigRoot string = ""
        ConfigSources struct = struct()
        ConfigCommands struct = struct()
    ...
    methods (Access = private)
        ...
        % Lightweight JSON configuration layer
        % =================================================================
        function initializeConfigLiteLayer(app)
            app.ConfigRoot = app.locateConfigRoot();
            app.ConfigSources = app.readJsonFile(fullfile(app.ConfigRoot, "config", "sources.json"));
            app.ConfigCommands = app.readJsonFile(fullfile(app.ConfigRoot, "config", "commands.json"));
            app.refreshConfigDrivenUI();
            app.configLog("[CONFIG] UI refreshed from JSON. Sources=" + ...
                strjoin(app.configSourceNames(false), ",") + "; Packets=" + ...
                strjoin(app.configPacketTypeNames(), ","));
        end

        function root = locateConfigRoot(app) %#ok<MANU>
            candidates = strings(0,1);

            try
                if evalin('base', "exist('MFC_LITE_ROOT','var')")
                    candidates(end+1,1) = string(evalin('base','MFC_LITE_ROOT'));
                end
            catch
            end
            ...
            try
                ...
                thisFolder = string(fileparts(thisFile));
                candidates(end+1,1) = thisFolder;
                candidates(end+1,1) = string(fileparts(thisFolder));
            catch
            end
            ...
            for k = 1:numel(candidates)
                ...
                if isfile(fullfile(c, "config", "sources.json")) && ...
                        isfile(fullfile(c, "config", "commands.json"))
                    root = c;
                    return
                end
            end
        ...
        function data = readJsonFile(app, filePath) %#ok<INUSD>
            if ~isfile(filePath)
        ...
        function configLog(app, msg)
            try
                old = app.SystemStatusTextArea.Value;
                if ischar(old) || isstring(old)
                    old = cellstr(old);
                end
                ts = char(string(datetime('now','Format','HH:mm:ss.SSS')));
                app.SystemStatusTextArea.Value = [old(:); {char("[" + ts + "] " + string(msg))}];
                drawnow limitrate
            catch
            end
        end

        function src = configModuleSourceStruct(app)
            if isempty(fieldnames(app.ConfigSources))
                src = struct([]);
                return
            end

            if isfield(app.ConfigSources, 'sources')
                src = app.ConfigSources.sources;
            elseif isfield(app.ConfigSources, 'moduleSources')
                src = app.ConfigSources.moduleSources;
            else
                src = struct([]);
            end
            ...
            for k = 1:numel(src)
                if isfield(src(k), 'enabled')
                    keep(k) = logical(src(k).enabled);
                end
            end
            src = src(keep);
        end

        function b = configBroadcastSourceStruct(app)
            b = struct('name', "all", 'address', 99, 'displayName', "all", 'enabled', true);
            if isfield(app.ConfigSources, 'broadcast')
                b0 = app.ConfigSources.broadcast;
                if isfield(b0, 'name'), b.name = string(b0.name); end
                if isfield(b0, 'address'), b.address = double(b0.address); end
                if isfield(b0, 'displayName'), b.displayName = string(b0.displayName); end
                if isfield(b0, 'enabled'), b.enabled = logical(b0.enabled); end
            end
        end

        function names = configSourceNames(app, includeAll)
            if nargin < 2
                includeAll = false;
            end
            s = app.configModuleSourceStruct();
            names = strings(0,1);
            for k = 1:numel(s)
                if isfield(s(k), 'name')
                    names(end+1,1) = lower(strtrim(string(s(k).name))); %#ok<AGROW>
                end
            end
            if includeAll
                b = app.configBroadcastSourceStruct();
                if isfield(b, 'enabled') && logical(b.enabled)
                    names(end+1,1) = lower(strtrim(string(b.name))); %#ok<AGROW>
                end
            end
            names = unique(names, 'stable');
        end

        function labels = configSourceDisplayNames(app, includeAll)
            if nargin < 2
                includeAll = false;
            end
            s = app.configModuleSourceStruct();
            labels = strings(0,1);
            for k = 1:numel(s)
                if isfield(s(k), 'displayName')
                    labels(end+1,1) = string(s(k).displayName); %#ok<AGROW>
                elseif isfield(s(k), 'name')
                    labels(end+1,1) = string(s(k).name); %#ok<AGROW>
                end
            end
            if includeAll
                b = app.configBroadcastSourceStruct();
                if isfield(b, 'enabled') && logical(b.enabled)
                    labels(end+1,1) = string(b.displayName); %#ok<AGROW>
                end
            end
        end

        function addr = configSourceAddress(app, source)
            addr = NaN;
            source = lower(strtrim(string(source)));
            s = app.configModuleSourceStruct();
            for k = 1:numel(s)
                if isfield(s(k), 'name') && lower(string(s(k).name)) == source
                    addr = double(s(k).address);
                    return
                end
            end
            b = app.configBroadcastSourceStruct();
        ...
        function name = configSourceNameFromAddress(app, addr)
            name = "source" + string(addr);
            s = app.configModuleSourceStruct();
            for k = 1:numel(s)
                if isfield(s(k), 'address') && double(s(k).address) == double(addr)
                    name = lower(string(s(k).name));
                    return
                end
            end
        end

        function tf = configIsModuleSourceName(app, name)
            tf = any(lower(strtrim(string(name))) == app.configSourceNames(false));
        end

        function names = configPacketTypeNames(app)
            names = strings(0,1);
            if ~isfield(app.ConfigCommands, 'packetTypes')
                return
            end
            p = app.ConfigCommands.packetTypes;
            for k = 1:numel(p)
                if isfield(p(k), 'name')
                    names(end+1,1) = string(p(k).name); %#ok<AGROW>
                end
            end
        end

        function pkt = configPacketDefByName(app, pktType)
            pkt = struct();
            if ~isfield(app.ConfigCommands, 'packetTypes')
                return
            end
            ...
            p = app.ConfigCommands.packetTypes;
            for k = 1:numel(p)
                if isfield(p(k), 'name') && lower(string(p(k).name)) == pktType
                    pkt = p(k);
                    return
                end
            end
        ...
        function pkt = configPacketDefByMessageType(app, msgType)
            pkt = struct();
            if ~isfield(app.ConfigCommands, 'packetTypes')
                return
            end
            p = app.ConfigCommands.packetTypes;
            for k = 1:numel(p)
                if isfield(p(k), 'messageType') && double(p(k).messageType) == double(msgType)
                    pkt = p(k);
                    return
                end
            end
        ...
        function code = configPacketCode(app, pktType)
            code = "";
            pkt = app.configPacketDefByName(pktType);
            if isfield(pkt, 'code')
                code = string(pkt.code);
            end
        end

        function refreshConfigDrivenUI(app)
            % Source selector in Packet Tester includes broadcast/all.
            try
                labels = cellstr(app.configSourceDisplayNames(true));
                app.ProductArrayListBox.Items = labels;
                if any(strcmpi(labels, 'all'))
                    app.ProductArrayListBox.Value = labels{strcmpi(labels,'all')};
                elseif ~isempty(labels)
                    app.ProductArrayListBox.Value = labels{1};
                end
            catch ME
                app.configLog("[CONFIG] ProductArrayListBox update failed: " + string(ME.message));
            end

            % Packet request dropdown from commands.json.
            try
                pktTypes = cellstr(app.configPacketTypeNames());
                app.PacketTypeDropDown.Items = pktTypes;
                if ~isempty(pktTypes)
                    app.PacketTypeDropDown.Value = pktTypes{1};
                end
            catch ME
                app.configLog("[CONFIG] PacketTypeDropDown update failed: " + string(ME.message));
            end

            % Source trees exclude broadcast/all. They select physical sources.
            app.refreshSourceTreeFromConfig(app.Tree_6, false);   % Monitor
            app.refreshSourceTreeFromConfig(app.Tree_9, false);   % Logger
            app.refreshSourceTreeFromConfig(app.Tree_14, false);  % Control
            app.refreshSourceTreeFromConfig(app.Tree_16, false);  % Scheduled Events

            app.refreshPacketTreeFromConfig(app.Tree_5);          % Monitor packets
            app.refreshPacketTreeFromConfig(app.Tree_8);          % Logger packets

            % Signal browsers are pre-populated from command packet structure.
            app.refreshSignalBrowserFromConfig(app.Tree_2, false);
            app.refreshSignalBrowserFromConfig(app.Tree_7, false);
        end

        function refreshSourceTreeFromConfig(app, treeObj, includeAll)
            try
                delete(treeObj.Children);
            catch
            end
            names = app.configSourceNames(includeAll);
            for k = 1:numel(names)
                n = uitreenode(treeObj, 'Text', char(names(k)));
                n.NodeData = struct('SourceName', names(k), 'Address', app.configSourceAddress(names(k)));
            end
            try
        ...
        function refreshPacketTreeFromConfig(app, treeObj)
            try
                delete(treeObj.Children);
            catch
            end
            pktTypes = app.configPacketTypeNames();
            for k = 1:numel(pktTypes)
                n = uitreenode(treeObj, 'Text', char(pktTypes(k)));
                pkt = app.configPacketDefByName(pktTypes(k));
                n.NodeData = pkt;
            end
            try
                treeObj.CheckedNodes = [];
        ...
        function refreshSignalBrowserFromConfig(app, treeObj, includeAll)
            try
                delete(treeObj.Children);
            catch
            end
            pktTypes = app.configPacketTypeNames();
            srcNames = app.configSourceNames(includeAll);
            for i = 1:numel(pktTypes)
                pktNode = uitreenode(treeObj, 'Text', char(pktTypes(i)));
                pkt = app.configPacketDefByName(pktTypes(i));
                for j = 1:numel(srcNames)
                    srcNode = uitreenode(pktNode, 'Text', char(srcNames(j)));
                    if isfield(pkt, 'fields')
                        for k = 1:numel(pkt.fields)
                            fName = string(pkt.fields(k).name);
                            fNode = uitreenode(srcNode, 'Text', char(fName));
                            fNode.NodeData = pkt.fields(k);
                        end
                    end
                end
            end
        ...
        function sources = getCheckedConfiguredSources(app, treeObj, includeAll)
            sources = strings(0,1);
            checked = treeObj.CheckedNodes;
            allowed = app.configSourceNames(includeAll);
            for k = 1:numel(checked)
                txt = lower(strtrim(string(checked(k).Text)));
                if any(txt == allowed)
                    sources(end+1,1) = txt; %#ok<AGROW>
                end
            end
            sources = unique(sources, 'stable');
        end

        function pktTypes = getCheckedConfiguredPackets(app, treeObj)
            pktTypes = strings(0,1);
            checked = treeObj.CheckedNodes;
            allowed = lower(app.configPacketTypeNames());
            realNames = app.configPacketTypeNames();
            for k = 1:numel(checked)
                txt = lower(strtrim(string(checked(k).Text)));
                idx = find(txt == allowed, 1, 'first');
                if ~isempty(idx)
                    pktTypes(end+1,1) = realNames(idx); %#ok<AGROW>
                end
            end
            pktTypes = unique(pktTypes, 'stable');
        end

        function [fields, units] = decodePayloadByDefinition(app, payload, pktDef)
            payload = payload(:);
            fields = struct();
            units = struct();

            expected = NaN;
            if isfield(pktDef, 'expectedPayloadLength')
                expected = double(pktDef.expectedPayloadLength);
            elseif isfield(pktDef, 'payloadLength')
                expected = double(pktDef.payloadLength);
            end

            if ~isnan(expected) && numel(payload) ~= expected
                error("%s payload length mismatch: expected %d, got %d", string(pktDef.name), expected, numel(payload));
            end

            i = 1;
            if ~isfield(pktDef, 'fields')
                fields.raw_payload = mat2str(payload(:).');
                units.raw_payload = "";
                return
            end

            for k = 1:numel(pktDef.fields)
                f = pktDef.fields(k);
                name = matlab.lang.makeValidName(char(string(f.name)));
                type = lower(strtrim(string(f.type)));
                [value, i] = app.readConfiguredPayloadValue(payload, i, type);
                fields.(name) = value;
                if isfield(f, 'unit')
                    units.(name) = string(f.unit);
                else
                    units.(name) = "";
                end
            end
        ...
        function [value, nextIndex] = readConfiguredPayloadValue(app, payload, index, type)
            switch lower(string(type))
                case {"uint8","u8","byte"}
                    [value, nextIndex] = app.readU8(payload, index);
                case {"uint16","u16"}
                    [value, nextIndex] = app.readU16(payload, index);
                case {"uint32","u32"}
                    [value, nextIndex] = app.readU32(payload, index);
                case {"float32","single","f32"}
                    [value, nextIndex] = app.readF32(payload, index);
                otherwise
                    error("Unsupported configured payload field type: %s", string(type));
            end
        end
...
function cmd = buildPacketRequestCommand(app, source, pktType)
    cmd = "";
    addr = app.configSourceAddress(source);
    typeCode = app.configPacketCode(pktType);
    ...
    if isfield(app.ConfigCommands, 'packetRequest')
        pr = app.ConfigCommands.packetRequest;
        if isfield(pr, 'start'), startToken = string(pr.start); end
        if isfield(pr, 'end'), endToken = string(pr.end); end
        if isfield(pr, 'format'), fmt = string(pr.format); end
    end

    cmd = string(fmt);
    cmd = strrep(cmd, "${start}", startToken);
    cmd = strrep(cmd, "${source}", string(addr));
    cmd = strrep(cmd, "${code}", typeCode);
    cmd = strrep(cmd, "${end}", endToken);
end
...
function decodedPkt = decodePacketFrame(app, frame)
    ...
    decodedPkt.Source      = app.configSourceNameFromAddress(sourceAddr);
    decodedPkt.PayloadLen  = payloadLen;
    decodedPkt.LocalTime   = datetime('now');
    decodedPkt.Fields      = struct();
    decodedPkt.Units       = struct();

    pktDef = app.configPacketDefByMessageType(msgType);
    if isfield(pktDef, 'name')
        decodedPkt.PacketType = string(pktDef.name);
        [decodedPkt.Fields, decodedPkt.Units] = app.decodePayloadByDefinition(payload, pktDef);
    else
...
function ok = sendPayloadToTarget(app, target, payload)
    ...
    target  = lower(strtrim(string(target)));

    if target == "all"
        b = app.configBroadcastSourceStruct();
        targets = double(b.address);
    else
        addr = app.configSourceAddress(target);
        if isnan(addr)
    ...
    for k = 1:numel(targets)
        cmd = app.buildMFCCommand(targets(k), payload);
        if strlength(cmd) == 0
            ok = false;
            app.setConnectionStatus("Failed to build command");
            return
        end
        thisOK = app.sendRawCommand(cmd);
        ok = ok && thisOK;
        pause(0.01);
    end
end
...
function pktTypes = getCheckedPacketTypes(app)
    pktTypes = app.getCheckedConfiguredPackets(app.Tree_5);
end

function sources = getCheckedSources(app)
    sources = app.getCheckedConfiguredSources(app.Tree_6, false);
end
...
function unitLabel = getUnitForSignalKey(app, key)
    ...
    if isfield(pkt, 'fields')
        for k = 1:numel(pkt.fields)
            if lower(string(pkt.fields(k).name)) == lower(string(field))
                if isfield(pkt.fields(k), 'unit') && strlength(string(pkt.fields(k).unit)) > 0
                    unitLabel = string(pkt.fields(k).unit);
                end
                return
            end
        end
...
function pktTypes = getCheckedLoggerPacketTypes(app)
    pktTypes = app.getCheckedConfiguredPackets(app.Tree_8);
end

function sources = getCheckedLoggerSources(app)
    sources = app.getCheckedConfiguredSources(app.Tree_9, false);
end
...
function rawCmd = buildRawControlCommand(app, cmdDef, source, valueStr)
    addrNum = app.configSourceAddress(source);
    if isnan(addrNum)
        rawCmd = "";
        return
    end
    ...
    switch inputMode
        ...
        otherwise
            if useSub
                rawCmd = "$" + addr + prefix + subcmd + string(valueStr) + "#";
            else
                rawCmd = "$" + addr + prefix + string(valueStr) + "#";
            end
    end
end
...
function source = getSelectedControlSource(app)
    source = "";
    try
        checkedNodes = app.getControlSourceTree().CheckedNodes;
    catch
        return
    end
    if isempty(checkedNodes), return; end
    allowed = app.configSourceNames(false);
    for k = 1:numel(checkedNodes)
        txt = lower(strtrim(string(checkedNodes(k).Text)));
        if any(txt == allowed)
            source = txt;
            return
        end
        ...
        function initializeRequestScheduler(app)
            ...
            sources = app.configSourceNames(false);
            for i = 1:numel(sources)
                src = sources(i);
                app.RequestScheduler.(char(src)) = struct( ...
                    'Source', src, ...
        ...
        function ok = sendPacketRequest(app, source, pktType)
            ...
            if ok
                app.logEvent("[SCHED REQ] Source=" + string(source) + ...
                    ", Type=" + string(pktType) + ...
                    ", Interval=" + string(app.RequestScheduler.(char(source)).Interval) + " s");
            end
        end
        function configureRequestSchedulerFromMonitorSelection(app, pktTypes, sources)
            ...
            pktTypes = lower(string(pktTypes));
            sources  = lower(string(sources));
            allSources = app.configSourceNames(false);

            % Add newly configured sources if sources.json changed after startup.
...
function source = getSelectedScheduleSource(app)
    source = "";
    checked = app.Tree_16.CheckedNodes;
    if isempty(checked), return; end
    allowed = app.configSourceNames(false);
    for k = numel(checked):-1:1
        txt = lower(strtrim(string(checked(k).Text)));
        if any(txt == allowed)
            source = txt;
            return
    ...
    methods (Access = private)
        ...
        function startupFcn(app)
            ...
            % Lightweight config-driven source/command refresh
            try
                app.initializeConfigLiteLayer();
            catch ME
                app.SystemStatusTextArea.Value = [app.SystemStatusTextArea.Value; {['[CONFIG ERROR] ' ME.message]}];
            end
            ...
            % Final config refresh after legacy startup cleared/rebuilt trees.
            try
                app.refreshConfigDrivenUI();
            catch ME
                app.configLog("[CONFIG] Final UI refresh failed: " + string(ME.message));
            end
        ...
        function StartplotterButtonPushed(app, event)
            ...
            if isempty(app.PlotterRefreshTimer) || ~isvalid(app.PlotterRefreshTimer)
                ...
                app.PlotterRefreshTimer.Period = app.MonitorRequestInterval;
            end
