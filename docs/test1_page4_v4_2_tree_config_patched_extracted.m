classdef test1_page4_v4_2 < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function cmd = buildPacketRequestCommand(app, source, pktType)
            ...
            % No source or packet type names should be hard-coded here.
            ...
            if strlength(addr) == 0 || strlength(typeCode) == 0
                return
            end

            cmd = string(fmt);
            cmd = strrep(cmd, "${start}", "$");
            cmd = strrep(cmd, "${source}", addr);
        ...
        function ok = sendPayloadToTarget(app, target, payload)
            ...
            switch target
                ...
                case "MFC4"
                    targets = 4;
                case "ALL"
                    targets = [1 2 3 4];   % better than 99 for LED effects unless you know 99 works reliably here
                otherwise
            ...
            for k = 1:numel(targets)
                ...
                pause(0.01);   % small gap between commands
            end
        end
        ...
        function pktTypes = getCheckedPacketTypes(app)
            % Monitor packet selector = Tree_5. Values come from NodeData,
            % which is filled from config/commands.json at startup.
            pktTypes = app.getCheckedTreeValuesLite("Tree_5");
        end

        function sources = getCheckedSources(app)
            % Monitor source selector = Tree_6. Values come from NodeData,
            % which is filled from config/sources.json at startup.
        ...
        function node = findChildNodeByText(app, parentNode, txt) %#ok<INUSD>
            ...
            for k = 1:numel(kids)
                if strcmpi(string(kids(k).Text), string(txt))
                    node = kids(k);
                    return
                end
            end
        ...
        function applyMonitorSelectionChange(app)
            ...
            % Then rebuild Tree_2 / Tree_7 from JSON packet definitions.
            app.ActiveSignalCatalog = struct();
            app.clearSignalBrowser();
        ...
        function pktTypes = getCheckedLoggerPacketTypes(app)
            % Logger packet selector = Tree_8. Values come from NodeData,
            % which is filled from config/commands.json at startup.
            pktTypes = app.getCheckedTreeValuesLite("Tree_8");
        end

        function sources = getCheckedLoggerSources(app)
            % Logger source selector = Tree_9. Values come from NodeData,
            % which is filled from config/sources.json at startup.
        ...
        function rawCmd = buildRawControlCommand(app, cmdDef, source, valueStr)
            % Build raw control command using config/sources.json for source address.
        ...
        function source = getSelectedControlSource(app)
            % Control source selector = Tree_14. Single-source command for now.
        ...
        function configureRequestSchedulerFromMonitorSelection(app, pktTypes, sources)
            ...
            allSources = ["mfc1","mfc2","mfc3","mfc4"];
        ...
        function source = getSelectedScheduleSource(app)
            % Schedule source selector = Tree_16. Single-source command for now.
        ...
        function raw = buildScheduledRawCommand(app, src, cmdKey, value)
            % Build scheduled control command using config/sources.json for source address.
            ...
            addr = app.sourceToAddressLite(src);
            if strlength(addr) == 0
                return
            end
        ...
        function refreshConfigDrivenUi(app)
            ...
            % Tree_2 and Tree_7 depend on both selected sources/packet types and
            % packet field definitions. Build them after all selector trees exist.
            app.refreshMonitorSignalBrowserFromConfigLite();
            app.refreshLoggerSignalBrowserFromConfigLite();
        ...
        function [names, labels, broadcastName, broadcastLabel] = getSourcesFromJsonLite(app, cfg)
            ...
            for k = 1:numel(src)
                ...
                if enabled
                    names(end+1) = lower(string(src(k).name)); %#ok<AGROW>
            ...
            if isfield(cfg, "broadcast")
                ...
                if enabled
                    broadcastName = lower(string(b.name));
        ...
        function [packetNames, packetLabels] = getPacketTypesFromJsonLite(app, cfg)
            ...
            % "packetTypes": [{"name":"Instantaneous","code":"I",...}, ...]
            ...
            % Simple legacy style:
            % "packetRequests": {"Instantaneous":"I", "Averaged":"A"}
        ...
        function sources = getCheckedTreeSourcesLite(app, treeName)
            % Return checked source keys from a JSON-filled source tree.
            sources = strings(0);
            ...
            try
                ...
                for k = 1:numel(nodes)
                    if ~isempty(nodes(k).NodeData)
                        sources(end+1) = lower(string(nodes(k).NodeData)); %#ok<AGROW>
                    else
                        sources(end+1) = lower(strtrim(string(nodes(k).Text))); %#ok<AGROW>
                    end
                end
        ...
        function values = getCheckedTreeValuesLite(app, treeName)
            % Return checked values from a JSON-filled packet/source tree.
            values = strings(0);
            ...
            try
                ...
                for k = 1:numel(nodes)
                    if ~isempty(nodes(k).NodeData)
                        ...
                        values(end+1) = strtrim(string(nodes(k).Text)); %#ok<AGROW>
                    end
                end
        ...
        function [names, labels] = physicalSourceNamesLite(app)
            ...
            [names, labels] = app.getSourcesFromJsonLite(cfg);
        end

        function cfg = readSourcesConfigLite(app)
            projectRoot = app.getProjectRootLite();
            cfg = jsondecode(fileread(fullfile(projectRoot, "config", "sources.json")));
        end

        function cfg = readCommandsConfigLite(app)
            projectRoot = app.getProjectRootLite();
            cfg = jsondecode(fileread(fullfile(projectRoot, "config", "commands.json")));
        end
        ...
        function addr = sourceToAddressLite(app, source)
            ...
            if isfield(cfg, "broadcast")
                b = cfg.broadcast;
                if lower(string(b.name)) == source || source == "99" || source == "broadcast"
                    addr = string(b.address);
                    return
                end
            end

            % Fallback for mfc12 style names, still config-independent.
            token = regexp(char(source), '\d+', 'match', 'once');
            if ~isempty(token)
                addr = string(token);
            end
        end
        ...
        function code = packetTypeToCodeLite(app, pktType)
            ...
            if isfield(cfg, "packetTypes")
                ...
                for k = 1:numel(p)
                    if lower(string(p(k).name)) == pktType
                        if isfield(p(k), "code")
                            code = string(p(k).code);
                        elseif isfield(p(k), "requestCode")
                            code = string(p(k).requestCode);
                        elseif isfield(p(k), "commandCode")
                            code = string(p(k).commandCode);
                        end
                        return
                    end
                end
            ...
            if isfield(cfg, "packetRequests")
                f = fieldnames(cfg.packetRequests);
                for k = 1:numel(f)
                    if lower(string(f{k})) == pktType
                        code = string(cfg.packetRequests.(f{k}));
                        return
                    end
                end
            end
        ...
        function packetDefs = getPacketDefinitionsLite(app)
            ...
            if isfield(cfg, "packetRequests")
                ...
                for k = 1:numel(names)
                    ...
                    packetDefs(k).code = string(cfg.packetRequests.(names{k}));
        ...
        function refreshSignalBrowserTreeLite(app, signalTreeName, sourceTreeName, packetTreeName)
            % Rebuild Tree_2 or Tree_7 from selected sources, selected packet
            % types, and packet fields defined in config/commands.json.
            ...
            for p = 1:numel(packetTypes)
                ...
                pktNode = uitreenode(signalTree, ...
                    "Text", char(pktName), ...
                    "NodeData", char(lower(pktName)));

                for s = 1:numel(sources)
                    srcName = lower(string(sources(s)));

                    srcNode = uitreenode(pktNode, ...
                        "Text", char(srcName), ...
                        "NodeData", char(lower(pktName) + "|" + srcName));
                    ...
                    if isfield(pktDef, "fields")
                        ...
                        for f = 1:numel(fields)
                            ...
                            if isfield(fields(f), "displayName")
                                fieldLabel = string(fields(f).displayName);
                            else
                                fieldLabel = fieldName;
                            end

                            sigKey = lower(pktName) + "|" + srcName + "|" + fieldName;
                            uitreenode(srcNode, ...
                                "Text", char(fieldLabel), ...
                                "NodeData", char(sigKey));
                        end
                    end
                end
            end
            ...
            try
                signalTree.Enable = 'on';
                expand(signalTree);
            catch
            end
        end

        function LoggerSelectionChangedLite(app, event) %#ok<INUSD>
            % Keep Logger signal browser in sync with source/packet selections.
            try
                app.refreshLoggerSignalBrowserFromConfigLite();
            catch ME
                app.logConfigLite("[CONFIG] Logger signal browser refresh failed: " + string(ME.message));
            end
            try
                app.updateLoggerStartButtonState();
            catch
    ...
    methods (Access = private)
        ...
        function startupFcn(app)
            ...
            app.Tree_8.CheckedNodesChangedFcn = createCallbackFcn(app, @LoggerSelectionChangedLite, true);
            app.Tree_9.CheckedNodesChangedFcn = createCallbackFcn(app, @LoggerSelectionChangedLite, true);
            app.updateLoggerStartButtonState();
