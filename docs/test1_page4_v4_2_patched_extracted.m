...
function cmd = buildPacketRequestCommand(app, source, pktType)
            % Build packet request command from config/sources.json and config/commands.json.
            %
            % Examples:
            %   mfc1 + Instantaneous -> $1I#
            %   mfc6 + Diagnostic    -> $6D#
            %   all  + Averaged      -> $99A#
            ...
            cmd = string(fmt.Format);
            cmd = strrep(cmd, "${start}", fmt.Start);
            cmd = strrep(cmd, "${source}", string(addr));
            cmd = strrep(cmd, "${code}", string(typeCode));
            cmd = strrep(cmd, "${end}", fmt.End);
        end
...
function ok = sendPayloadToTarget(app, target, payload)
            ...
            if isempty(targets)
                app.setConnectionStatus("No enabled source targets found");
                return
            end
...
function pktTypes = getCheckedPacketTypes(app)
            ...
            % Runtime nodes are rebuilt from config/commands.json.

            pktTypes = app.getCheckedTreeValuesLite("Tree_5");
            pktTypes = unique(string(pktTypes), 'stable');
        end

        function sources = getCheckedSources(app)
            % Monitor source selector = Tree_6.
            % Runtime nodes are rebuilt from config/sources.json.
...
function rawCmd = buildRawControlCommand(app, cmdDef, source, valueStr)
            % Build raw control command using source address from sources.json.
...
function source = getSelectedControlSource(app)
            ...
            % Return the first checked JSON source key.
...
function initializeRequestScheduler(app)
            % Initialize request scheduler entries from config/sources.json.
            % Broadcast/all is not represented here; this scheduler iterates
            % over physical sources selected in Monitor/Logger UI.
...
function configureRequestSchedulerFromMonitorSelection(app, pktTypes, sources)
            % Configure request scheduler using JSON-defined physical sources.
            ...
            allSources = app.physicalSourceNamesLite();

            % Add newly configured sources if JSON changed while app is running.
...
function source = getSelectedScheduleSource(app)
            ...
            % Return the last checked JSON source key to preserve previous UI behavior.
...
function [names, labels, broadcastName, broadcastLabel] = getSourcesFromJsonLite(app, cfg)
            % Read physical sources and optional broadcast source from sources.json.
            %
            % Supported lightweight schemas:
            %   {"sources":[...], "broadcast":{...}}
            %   {"moduleSources":[...], "broadcast":{...}}
            %
            % UI source trees use only physical sources.
            % Packet tester source list can additionally include broadcast/all.
...
function [packetNames, packetLabels] = getPacketTypesFromJsonLite(app, cfg)
            % Read packet type names from commands.json.
            %
            % Supported schemas:
            %   packetTypes = [{"name":"Instantaneous", ...}, ...]
            %   packetRequests = {"Instantaneous":"I", ...}
        ...
        function sources = getCheckedTreeSourcesLite(app, treeName)
            % Return checked source names from a JSON-built CheckBoxTree.
            % NodeData is the config key. Text is only the display label.
            ...
            try
                checkedNodes = app.(treeName).CheckedNodes;
            catch
                return
            end
            ...
            for k = 1:numel(checkedNodes)
                ...
                try
                    if ~isempty(checkedNodes(k).NodeData)
                        val = string(checkedNodes(k).NodeData);
                    end
                catch
                end

                if strlength(val) == 0
                    val = string(checkedNodes(k).Text);
                end

                if strlength(val) > 0
                    sources(end+1,1) = lower(strtrim(val)); %#ok<AGROW>
                end
            end
        ...
        function values = getCheckedTreeValuesLite(app, treeName)
            % Return checked values from any JSON-built CheckBoxTree.
            % Uses NodeData when available, otherwise falls back to Text.
            ...
            for k = 1:numel(checkedNodes)
                ...
                if strlength(val) > 0
                    values(end+1,1) = strtrim(val); %#ok<AGROW>
                end
            end
        ...
        function srcCfg = readSourcesConfigLite(app)
            projectRoot = app.getProjectRootLite();
            sourcesFile = fullfile(projectRoot, "config", "sources.json");
        ...
        function cmdCfg = readCommandsConfigLite(app)
            projectRoot = app.getProjectRootLite();
            commandsFile = fullfile(projectRoot, "config", "commands.json");
        ...
        function addr = sourceToAddressLite(app, source)
            % Resolve source key/display text to numeric firmware address.
            % Reads config/sources.json every time so JSON edits apply after app restart.
            ...
            for k = 1:numel(src)
                ...
                name = lower(string(src(k).name));
                displayName = name;
                if isfield(src(k), "displayName")
                    displayName = lower(string(src(k).displayName));
                end

                if source == name || source == displayName
            ...
            if isfield(cfg, "broadcast")
                ...
                if enabled
                    name = lower(string(b.name));
                    displayName = name;
                    if isfield(b, "displayName")
                        displayName = lower(string(b.displayName));
                    end

                    if source == name || source == displayName || source == "99" || source == "broadcast"
            ...
            if ~isempty(token)
                addr = str2double(token);
            end
        end

        function addresses = physicalSourceAddressesLite(app)
            % Return addresses for enabled physical sources only.
            % Broadcast/all is intentionally excluded.
        ...
        function sourceNames = physicalSourceNamesLite(app)
            % Return enabled physical source names from sources.json.
            ...
            for k = 1:numel(src)
                ...
                if enabled
                    sourceNames(end+1,1) = lower(string(src(k).name)); %#ok<AGROW>
                end
            end
        end

        function code = packetTypeToCodeLite(app, pktType)
            % Resolve packet type name to command letter from config/commands.json.
            ...
            if isfield(cfg, "packetTypes")
                ...
                for k = 1:numel(p)
                    name = lower(string(p(k).name));
                    displayName = name;

                    if isfield(p(k), "displayName")
                        displayName = lower(string(p(k).displayName));
                    end

                    if pktType == name || pktType == displayName
        ...
        function fmt = packetRequestFormatLite(app)
            % Return packet request command format information.
            ...
            fmt = struct( ...
                'Start', "$", ...
                'End', "#", ...
                'Format', "${start}${source}${code}${end}");

            if isfield(cfg, "packetRequest")
                pr = cfg.packetRequest;

                if isfield(pr, "start")
                    fmt.Start = string(pr.start);
                end
                if isfield(pr, "end")
                    fmt.End = string(pr.end);
                end
                if isfield(pr, "format")
                    fmt.Format = string(pr.format);
                end
            end
        end
