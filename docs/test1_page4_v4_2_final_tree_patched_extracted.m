classdef test1_page4_v4_2 < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function pktTypes = getCheckedPacketTypes(app)
            ...
            checkedNodes = app.Tree_5.CheckedNodes;
            if isempty(checkedNodes)
                return
            end
    
            for k = 1:numel(checkedNodes)
                name = strtrim(string(checkedNodes(k).Text));
    
                switch lower(name)
                    case "instantaneous"
                        pktTypes(end+1,1) = "Instantaneous";
                    case "averaged"
                        pktTypes(end+1,1) = "Averaged";
                    case "diagnostic"
                        pktTypes(end+1,1) = "Diagnostic";
                end
            end
        ...
        function sources = getCheckedSources(app)
            ...
            try
                nodes = app.Tree_6.CheckedNodes;
        
                for k = 1:numel(nodes)
                    nd = nodes(k).NodeData;
        
                    if ~isempty(nd)
                        sources(end+1) = string(nd); %#ok<AGROW>
                    else
                        sources(end+1) = string(nodes(k).Text); %#ok<AGROW>
                    end
                end
            catch
            end
        ...
        function pktTypes = getCheckedLoggerPacketTypes(app)
            ...
            checkedNodes = app.Tree_8.CheckedNodes;
            if isempty(checkedNodes)
                return
            end
        ...
        function sources = getCheckedLoggerSources(app)
            ...
            checkedNodes = app.Tree_9.CheckedNodes;
            if isempty(checkedNodes)
                return
            end
        
            for k = 1:numel(checkedNodes)
                name = lower(strtrim(string(checkedNodes(k).Text)));
        
                switch name
                    case "mfc1"
                        sources(end+1,1) = "mfc1";
                    case "mfc2"
                        sources(end+1,1) = "mfc2";
                    case "mfc3"
                        sources(end+1,1) = "mfc3";
                    case "mfc4"
                        sources(end+1,1) = "mfc4";
                end
            end
        ...
        function source = getSelectedControlSource(app)
            ...
            for k = 1:numel(checkedNodes)
                txt = lower(string(checkedNodes(k).Text));
                switch txt
                    case "mfc1"
                        source = "mfc1"; return
                    case "mfc2"
        ...
        function source = getSelectedScheduleSource(app)
            ...
            txt = lower(strtrim(string(checked(end).Text)));
        
            switch txt
                case {"mfc1","mfc2","mfc3","mfc4"}
                    source = txt;
            end
        end
        ...
        function refreshSignalBrowserTreeLite(app, signalTreeName, sourceTreeName, packetTreeName)
            % Rebuilds Tree_2 or Tree_7 from:
            %   selected sources
            %   selected packet types
            %   packet fields in config/commands.json
            %
            % IMPORTANT: Preserve the original live-tree hierarchy used by
            % the plotter/logger selection code:
            %
            %   PacketType
            %       Source
            %           Field
            %
            % Existing functions such as getCheckedSignalKeysFromTree2() and
            % getCheckedLoggerSignalKeysFromTree7() assume:
            %   field node parent      = source node
            %   source node parent     = packet-type node
            %
            % Therefore do not change this hierarchy to Source -> Packet -> Field.
            ...
            % Read selected sources. If none selected, use all physical sources.
            ...
            % Read selected packet types. If none selected, use all packet types.
            ...
            % Clear old live/design-time contents.
            try
                ...
                for s = 1:numel(sources)
                    ...
                    if isfield(pktDef, "fields")
                        ...
                        for f = 1:numel(fields)
                            ...
                            % NodeData stores a stable machine key. Text remains
                            % the display field name so existing plot/logger code
                            % continues to work.
                            signalKey = lower(pktName) + "|" + lower(srcName) + "|" + fieldName;
        
                            uitreenode(srcNode, ...
                                "Text", char(fieldLabel), ...
                                "NodeData", char(signalKey));
                        end
                    end
                end
            end
        ...
        function sources = getCheckedTreeSourcesLite(app, treeName)
            % Return checked source keys from a CheckBoxTree.
            % Runtime JSON-created nodes store the source key in NodeData;
            % Text is used only as a fallback for older/static nodes.
